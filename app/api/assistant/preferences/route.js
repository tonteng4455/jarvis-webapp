...
export async function GET() {
  ...
  return NextResponse.json(result?.voiceName !== undefined ? result : { voiceName: null, voiceLang: null, personality: '' });
}
...
export async function POST(request) {
  ...
  const { voiceName, voiceLang, personality } = await request.json().catch(() => ({}));
  const result = await callBotInternal('/internal/voice-preferences', {
    userId: session.lineUserId,
    action: 'set',
    voiceName, voiceLang, personality,
  });
